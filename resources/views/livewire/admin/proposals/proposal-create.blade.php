            ...
            <div class="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
                <div class="flex items-start gap-4">
                    <div class="w-1/4">
                        ...
                        <button type="button" wire:click="$dispatch('openModal', {component: 'admin.features.feature-modal'})" class="button button-icon">
                        ...
                        @foreach($features as $feature)
                            <div wire:key="feature_{{ $feature->id }}">
                                <button wire:click="selectFeature({{ $feature->id }})" type="button" class="flex w-full items-center justify-between px-1 py-2 bg-gray-200 hover:bg-primary-100">
                    ...
                    <div class="w-3/4">
                        ...
                        <table class="min-w-full divide-y divide-gray-300">
                            ...
                            @foreach ($selectedFeatures as $selectedFeature)
                                <tr class="even:bg-gray-50 hover:bg-primary-50" wire:key="selected_{{ $selectedFeature->id }}">
                                    ...
                                    <td class="relative py-4 pr-4 pl-3 text-right text-sm font-medium whitespace-nowrap sm:pr-3 flex items-center gap-2 justify-end">
                                        <button wire:click="removeFeature({{ $selectedFeature->id }})" wire:confirm="Are you sure you wish to remove [{{ $selectedFeature->name }}]?" class="button button-warning">X<span class="sr-only">, Remove {{ $selectedFeature->name }}</span></button>
                                    </td>
                                </tr>
                            @endforeach
