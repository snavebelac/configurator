...
class ProposalTermsTest extends TestCase
{
    ...
    #[Test]
    public function delivering_pins_the_default_sets_current_version()
    {
        $terms = $this->termsSet('Standard build', '<p>The terms.</p>');
        $proposal = $this->proposal();

        $this->assertNull($proposal->terms_version_id);

        Livewire::test(ProposalEdit::class, ['proposal' => $proposal])
            ->call('markAsDelivered');

        $this->assertSame(
            $terms->currentVersion->id,
            $proposal->fresh()->terms_version_id,
        );
    }
    ...
    {
        $this->termsSet('Standard build', '<p>Payment is due within 30 days.</p>');
        $proposal = $this->proposal();
    ...
    public function a_version_attached_to_a_proposal_reports_itself_as_in_use()
    {
        ...
        $proposal = $this->proposal();
        Livewire::test(ProposalEdit::class, ['proposal' => $proposal])->call('markAsDelivered');
